...
export async function shopifyAdminQuery(query, variables = {}) {
  const shopDomain = process.env.SHOPIFY_STORE_DOMAIN || 'alldogsrockshop.myshopify.com';
  const accessToken = process.env.SHOPIFY_SECRET_KEY; // This is the admin API access token

  // Handle different domain formats
  let shopifyDomain = shopDomain;

  // If it already has .myshopify.com, use as-is
  if (!shopifyDomain.includes('.myshopify.com')) {
    // Remove www. and common TLDs, then add .myshopify.com
    shopifyDomain = shopifyDomain
      .replace('www.', '')
      .replace('.com', '')
      .replace('.myshopify', '') + '.myshopify.com';
  }

  const apiUrl = `https://${shopifyDomain}/admin/api/2024-10/graphql.json`;
